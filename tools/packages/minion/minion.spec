...
if [ "%{skip_compile}" = 1 ]; then
	...
	PROJECTS=":org.opennms.container.karaf,$(
		find features/minion -name pom.xml | while read FILE; do \
			grep artifactId $FILE | head -n 2; \
		done \
		| sed -e 's,.*<artifactId>,,' -e 's,</artifactId>,,' -e 's,^,org.opennms.features.minion:,' \
		| grep -v -E '^org.opennms.features.minion:org.opennms.features$' \
		| sort -u \
		| tr '\n' ',' \
		| sed -e 's/,$//' \
	)"
